...
export function ComparisonAnalysis({ url, earlierDate, laterDate, archiveUrl1, archiveUrl2 }: ComparisonAnalysisProps) {
  ...
  if (!analysis && !loading) {
    return (
      <div className="bg-gradient-to-r from-purple-900/20 to-blue-900/20 border border-purple-500/30 rounded-xl p-4 mt-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-gray-300">AI can analyze these snapshots</span>
          </div>
          <button
            onClick={analyze}
            className="px-4 py-2 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 rounded-lg text-sm font-medium transition-all"
          >
            Analyze Changes
          </button>
        </div>
  ...
  if (loading) {
    return (
      <div className="bg-gradient-to-r from-purple-900/20 to-blue-900/20 border border-purple-500/30 rounded-xl p-6 mt-4">
        <div className="flex items-center gap-3 mb-4">
          ...
          <span className="text-gray-300">Analyzing website evolution...</span>
        </div>
        ...
        {currentFact && (
          <div className="bg-gray-800/50 rounded-lg p-4 border border-gray-700/50">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-gray-300 text-sm">{currentFact.fact}</p>
                {currentFact.year && (
  ...
  return (
    <div className="bg-gradient-to-b from-gray-800/30 to-gray-900/30 border border-gray-700/50 rounded-xl p-6 mt-4">
      {/* Header */}
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 bg-gray-700/50 hover:bg-gray-600/50 rounded-lg text-sm transition-colors"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {analysis?.sentiment && (
        <div className="bg-gray-800/50 rounded-lg p-4 mb-4">
          <h4 className="text-sm font-medium text-gray-400 mb-3">Design Sentiment Trend</h4>
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <span className={`w-3 h-3 rounded-full ${getSentimentColor(analysis.sentiment.earlier)}`}></span>
              <span className="text-sm text-gray-300 capitalize">{analysis.sentiment.earlier}</span>
              <span className="text-gray-500 text-xs">({earlierDate})</span>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-gray-300 capitalize">{analysis.sentiment.later}</span>
              <span className="text-gray-500 text-xs">({laterDate})</span>
            </div>
          </div>
        ...
        <div className="mt-4 pt-4 border-t border-gray-700">
          ...
          <div className="grid gap-2">
            {analysis.actionableInsights.map((insight, i) => (
              <div key={i} className="bg-green-500/10 border border-green-500/20 rounded-lg p-3 text-sm text-gray-300">
                {insight}
              </div>
            ))}
          </div>
